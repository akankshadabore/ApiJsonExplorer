...
export default function JsonViewer({ jsonData, loading }) {
  ...
  const downloadJson = () => {
    const textToDownload = typeof jsonData === "string" 
      ? jsonData 
      : JSON.stringify(jsonData, null, 2);
        ...
        {jsonData && (
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="flex items-center gap-1 px-3 py-1 text-sm bg-green-600 hover:bg-green-500 text-white rounded transition-colors"
            >
              <FiDownload />
              Download
            </button>
...
function RenderJson({ data, indent = 0, expandedPaths, onTogglePath, path = "" }) {
  ...
  if (typeof data !== "object") {
    return <span className="text-orange-300">{JSON.stringify(data)}</span>;
  }
  ...
  return (
    <div>
      ...
      {isExpanded && entries.map(([key, value], index) => (
        <div key={index} style={{ marginLeft: `${indent + 20}px` }} className="flex flex-wrap">
          {!isArray && (
            <>
